...
````
I'll create the eks-deployer.md file with the content you provided.


🛠️  Using tool: fs_write (trusted)
 ⋮ 
 ● Path: /home/ec2-user/eks-deployer.md

+     1: 사용자가 "application" 또는 "app"이라고 언급할 경우, 이는 ~/environment/eks-workshop-v2/manifests/base-application 경로에 위치한 애플리케이션을 의미합니다.
+     2: 애플리케이션에 대한 모든 변경 사항은 업데이트를 적용하기 전에 YAML 파일에서 수정되어야 합니다.
+     3: 업데이트를 적용할 때는 애플리케이션의 최상위 디렉터리에서 kustomize 플래그 -f 를 사용해야 합니다.
+     4: 애플리케이션 토폴로지 다이어그램을 생성할 때는 Markdown(.md) 파일로 작성하며, 아래와 같은 MkDocs용 mermaid 코드 스니펫을 포함해야 합니다:
+     5: ```mermaid
+     6: graph LR
+     7: A[Start] --> B{Error?};
+     8: B -->|Yes| C[Hmm...];
...
Creating: /home/ec2-user/eks-deployer.md

 ⋮ 
 ● Completed in 0.0s


The file has been created successfully. The eks-deployer.md file now contains the guidelines for working with applications in the EKS workshop environment, 
including:

• References to applications at ~/environment/eks-workshop-v2/manifests/base-application
• Instructions for modifying YAML files before applying updates
• Requirements to use kustomize with the -f flag from the application's top directory
• Guidelines for creating application topology diagrams with mermaid code snippets in Markdown
````
...
다음은 Amazon Q CLI를 사용해 EKS 애플리케이션을 검토, 배포, 상태 확인, 업데이트, 토폴로지 시각화하는 주요 프롬프트와 그 설명을 한국어로 정리한 것입니다:
...
```
Wait for all pods in the app for condition ready
```
