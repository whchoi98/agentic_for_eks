---
description: 'Update : 2025.05.09'
---

# 8. EKS 활용 사례 3 (Troubleshooting)
...
I'll help you check the service selector, pod labels, and endpoints for the catalog service in the catalog namespace.
...
NAME      ENDPOINTS          AGE
catalog   10.11.0.104:8080   3h13m
...
서비스 셀렉터와 파드 라벨이 정확히 일치하고 있습니다. 서비스 셀렉터에 정의된 모든 키-값 쌍(app.kubernetes.io/component, app.kubernetes.io/instance, 
app.kubernetes.io/name)이 파드 라벨에도 동일하게 존재합니다.

파드 라벨에는 추가적으로 app.kubernetes.io/created-by와 pod-template-hash 라벨이 있지만, 이는 서비스 셀렉터에 영향을 주지 않습니다. 서비스 셀렉터는 지정된 라벨이
모두 일치하는 파드를 찾는 것이기 때문입니다.

엔드포인트도 정상적으로 생성되어 있어 서비스가 파드로 트래픽을 올바르게 라우팅하고 있음을 확인할 수 있습니다.
```
...
```
현재 RabbitMQ와 orders 서비스의 상태를 확인한 결과입니다:

### RabbitMQ 상태
• RabbitMQ 파드(rabbitmq-0)가 정상적으로 실행 중입니다.
• StatefulSet은 1/1 Ready 상태로, 필요한 모든 파드가 실행 중입니다.

### Orders 서비스 상태
• Orders 파드(orders-79bc47b54d-qsb5f)가 정상적으로 실행 중입니다.
• Orders MySQL 파드도 정상적으로 실행 중입니다.

### 로그 분석
• Orders 서비스 로그에서 "mysql"과 "rabbitmq" 프로필이 활성화되어 있음을 확인했습니다.
• 현재 로그에서는 RabbitMQ 관련 오류가 발견되지 않았습니다.

이전에 발생했던 RabbitMQ 연결 문제는 파드 재시작을 통해 해결된 것으로 보입니다. 현재 모든 서비스가 정상적으로 작동하고 있으며, RabbitMQ 연결 관련 오류가 더 이상 
발생하지 않고 있습니다.
```
...
sed -i 's/ecr/erc/g' ~/environment/eks-workshop/base-application/assets/deployment.yaml

# 잘못된 디플로이 적용
kubectl apply -k ~/environment/eks-workshop/base-application/assets/
```
